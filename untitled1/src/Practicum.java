...
public class Practicum {
    public static void main(String[] args) {
        ...
        Map<String, Integer> films = new LinkedHashMap<>(actorsList.size(),75,false);
        int i=0;
        String prev = actorsList.get(i);
        int count = 0;
        for(String act : actorsList)
        {
            if(act.equals(prev) && count!=actorsList.size()-1)
                i++;
            else
            {
                if(films.containsKey(prev))
                    i+=films.get(prev);
                if(count==actorsList.size()-1)
                    i++;
                films.put(prev,i);
                i=1;
                prev=act;
